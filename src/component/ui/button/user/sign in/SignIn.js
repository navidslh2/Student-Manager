...
const SignIn = (props) => {
  ...
  const createCaptcha = ()=>{
    ...
    setStyle1({color:`rgba(${Math.random()*255}, ${Math.random()*255},${Math.random()*255})`,transform:`rotate(${Math.random()*90}deg)`})
    setStyle2({color:`rgba(${Math.random()*255}, ${Math.random()*255},${Math.random()*255})`,transform:`rotate(${Math.random()*90}deg)`})
    setStyle3({color:`rgba(${Math.random()*255}, ${Math.random()*255},${Math.random()*255})`,transform:`rotate(${Math.random()*90}deg)`})
    setStyle4({color:`rgba(${Math.random()*255}, ${Math.random()*255},${Math.random()*255})`,transform:`rotate(${Math.random()*90}deg)`})
  ...
  const validate = ()=>{
    const emailpattern = /^([a-zA-z0-9\.-]+)@([a-z0-9]+).([a-z]{2,5})(.[a-z]{2,5})?$/
  ...
  return (
    <>
      ...
      <div className="input-container">
        <input type="password" placeholder="password" onChange={passwordChangeHandler}  value={passwordValue} />
      </div>
      <div className="captcha-conteiner">
        ...
        <input type="text"  placeholder="enter the captcha " onChange={captchaHandler} value={captchaValue}/>
      </div>
      <Button btnType="green" clicked={loginHandler}>Login</Button>
    </>
  );
};
